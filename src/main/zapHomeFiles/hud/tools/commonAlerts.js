      ...
      .then(results => {
        ...
        apiCallWithResponse('alert', 'view', 'alertsByRisk', { url: target, recurse: 'false' })
          .then(json => {
            ...
            const raisedEventDetails = { domain: targetDomain, url: event.detail.uri, target: origTarget, pageAlerts: pageAlerts }
      ...
      if (save) {
        utils.loadTool(NAME)
          ...
          .then(() => {
            ...
            const raisedEventDetails = { risk: risk, domain: targetDomain, url: event.detail.uri, count: Object.keys(alertCache[targetDomain][risk]).length }
            utils.log(LOG_DEBUG, 'AlertEventPublisher eventListener', 'dispatchEvent ' + raisedEventName, raisedEventDetails)
